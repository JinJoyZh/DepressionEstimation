...
def sliding_window_text(fkps_coor, text_feature, text_feature_use, text_feature_usel, visual_sr, 
                        window_size, overlap_size, output_root, ID, trans_included=False):
    ...
    if trans_included:
        ...
        for i in range(num_frame):
            ...
            # frame_sample_gcc_trans = frame_sample_gcc
            frame_sample_text_trans = frame_sample_text
            frame_sample_text_use_trans = frame_sample_text_use
            frame_sample_text_usel_trans = frame_sample_text_usel
            ...
            # np.save(os.path.join(output_root, 'audio', 'spectrogram', f'{ID}-{i:02}_audio_trans.npy'), frame_sample_spec_trans)
            # np.save(os.path.join(output_root, 'audio', 'mel-spectrogram', f'{ID}-{i:02}_audio_trans.npy'), frame_sample_mspec_trans)
            np.save(os.path.join(output_root, 'text', 'sentencetransformer', f'{ID}-{i:02}_text_trans.npy'), frame_sample_text_trans)
            np.save(os.path.join(output_root, 'text', 'universal_sentence_encoder', f'{ID}-{i:02}_text_trans.npy'), frame_sample_text_use_trans)
            np.save(os.path.join(output_root, 'text', 'universal_sentence_encoder_large', f'{ID}-{i:02}_text_trans.npy'), frame_sample_text_usel_trans)
...
if __name__ == '__main__':
    ...
    gt_path = '/home/keyan/workspace/DepressionRec/dataset/DAIC-WOZ_dataset/train_split_Depression_AVEC2017.csv'
    ...
    for i in range(len(gt_df)):
        ...
        audio_path = f'/home/keyan/workspace/DepressionRec/dataset/all_audio_files/{patient_ID}_AUDIO.wav'
        audio_trans_path = f'/home/keyan/workspace/DepressionRec/dataset/all_audio_files/{patient_ID}_AUDIO_trans.wav'
        ...
        num_frame = sliding_window_text(filtered_fkps_coor, text_feature, 
                                        ...
                                        visual_sr, window_size, overlap_size, output_root, patient_ID, trans_included=False)
        ...
        num_frame = sliding_window_text(filtered_fkps_coor, text_feature, 
                                        text_feature_use, text_feature_usel, 
                                        visual_sr, window_size, overlap_size, output_root, patient_ID, trans_included=True)
